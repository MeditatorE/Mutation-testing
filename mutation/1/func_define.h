...
long double SIN(int number,long double count[],int len)
{
	...
	{
	   if((int)(count[0]/PI)<count[0]/PI)//if((int)(count[0]/PI)==count[0]/PI)
		    return sin(0.0);
		else 
		{
...
{
	if(function_array[number].std_number!=len&&count[0]>0)//if(function_array[number].std_number==len&&count[0]>0)
	{
		return log(count[0]);
	}
	return SINT;
